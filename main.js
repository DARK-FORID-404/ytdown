// main.js - YouTube Downloader with Quality Selection
// Uses ytdl-core streaming (handles token refresh automatically)
...
// Download using ytdl-core streaming (handles 410 errors)
async function downloadWithYtdl(videoUrl, itag, quality) {
  const info = await ytdl.getInfo(videoUrl);
  ...
  // Use ytdl stream (handles token refresh automatically)
...
<body style="font-family: Arial; max-width: 600px; margin: 50px auto; text-align: center; background: #0f0f0f; color: #fff;">
  ...
  <p style="color: #555; font-size: 13px;">Make sure the URL is valid and the video is public</p>
  ...
  // ============================================================
  // DOWNLOAD ENDPOINT - Uses ytdl stream (no 410 errors)
  // ============================================================
  if (path === "/download" && req.method === "GET") {
    ...
    try {
      const { stream, filename, contentType } = await downloadWithYtdl(videoUrl, itag, quality);
      const headers = {
      ...
      return new Response(JSON.stringify({ 
        error: err.message,
        tip: "Try using a different quality or check if the video is available"
      }), {
        status: 500,
        headers: { "Content-Type": "application/json", ...corsHeaders },
      });
  ...
  if (path === "/api/download" && req.method === "GET") {
    ...
    try {
      const { stream, filename, contentType } = await downloadWithYtdl(videoUrl, itag);
      const headers = {
        "Content-Disposition": `attachment; filename="${filename}"`,
        "Content-Type": contentType,
        ...corsHeaders,
...
console.log("📌 Usage: https://your-app.deno.dev/?url=YOUTUBE_URL");
